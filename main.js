...
function buildStem(params, grown) {
  ...
  const baseWidth = Number.isFinite(params.stemWidth) ? params.stemWidth : 0.095;
  const width = Math.max(0.01, baseWidth);

  const stem = new THREE.Mesh(
    new THREE.CylinderGeometry(width * 0.5, width, height, 12),
    createMaterial(params.stemColor, { roughness: 0.84 })
  );
...
function buildLeaves(params, grown) {
  ...
  if (params.leafCount === 0 || leafGrowth < 0.02) {
    return;
  }
  ...
  for (let i = 0; i < params.leafCount; i++) {
    ...
    for (let s = 0; s <= 20; s++) {
      ...
      point.add(
        sideDirection.clone().multiplyScalar(Math.sin(t * Math.PI * 2) * 0.025 * length)
      );
    ...
    const leaf = new THREE.Mesh(
      ...
      createMaterial(params.leafColor, { roughness: 0.7 })
    );

    add(leaf);

    if (params.leafPattern === "variegated") {
      const stripePoints = points.map((point) => point.clone().add(new THREE.Vector3(0, 0.003, 0)));
      const stripeWidths = widths.map((widthValue) => widthValue * 0.3);
      const stripe = new THREE.Mesh(
        makeRibbonGeometry(stripePoints, stripeWidths),
        createMaterial(params.variegationColor, { roughness: 0.62 })
      );
...
function buildBud(params, stemTop) {
  ...
  if (budGrowth < 0.02 || bloomOpening > 0.98) {
    return;
  }

  const budColor = new THREE.Color(params.stemColor).lerp(new THREE.Color(params.colorBase), 0.3);
  const bud = new THREE.Mesh(
    new THREE.SphereGeometry(0.25 * budGrowth, 18, 14),
    createMaterial(budColor, { roughness: 0.46, transparent: true, opacity: 1 - bloomOpening * 0.9 })
  );
    ...
    for (let i = 0; i < petalCount; i++) {
      ...
      const petal = new THREE.Mesh(
        ...
        createMaterial(petalColor, {
          roughness: 0.43,
          emissive: petalColor.clone().multiplyScalar(0.055),
        })
      );
...
function makeTube(points, radius, color) {
  ...
  return new THREE.Mesh(
    ...
    createMaterial(color, { roughness: 0.35 })
  );
}
...
function buildCoreStamensAndPollen(params, stemTop) {
  ...
  const core = new THREE.Mesh(
    new THREE.SphereGeometry(0.13 * coreGrowth, 16, 12),
    createMaterial(params.antherColor, {
      roughness: 0.35,
      emissive: new THREE.Color(params.antherColor).multiplyScalar(0.08),
    })
  );
  ...
  for (let i = 0; i < params.stamenCount; i++) {
    ...
    const anther = new THREE.Mesh(
      new THREE.SphereGeometry(0.052 * coreGrowth, 10, 8),
      createMaterial(params.antherColor, { roughness: 0.34 })
    );
  ...
  const pollenMaterial = createMaterial(params.pollenColor, {
    roughness: 0.3,
    emissive: new THREE.Color(params.pollenColor).multiplyScalar(0.05),
  });
  ...
  for (let i = 0; i < grainCount; i++) {
    ...
    const pollen = new THREE.Mesh(
      new THREE.SphereGeometry(params.pollenSize * coreGrowth, 7, 6),
      pollenMaterial
    );
...
function randomizeFlower() {
  ...
  if (ui.stemWidth) {
    const min = Number(ui.stemWidth.min || 0.02);
    const max = Number(ui.stemWidth.max || 0.2);
    const step = Number(ui.stemWidth.step || 0.01);
    const raw = min + Math.random() * (max - min);
    ui.stemWidth.value = String(Math.round(raw / step) * step);
  }
...
for (const id of ids) {
  if (ui[id]) {
    ui[id].addEventListener("input", rebuildFlower);
  }
}
...
ui.playPauseBtn?.addEventListener("click", () => {
  playing = !playing;
  ui.playPauseBtn.textContent = playing ? "Pause" : "Play";
});
...
container.addEventListener("pointermove", (event) => {
  ...
  yaw = dragStartYaw + (event.clientX - dragStartX) * 0.012;
});
...
container.addEventListener(
  ...
  (event) => {
    ...
    camera.position.z = THREE.MathUtils.clamp(camera.position.z + event.deltaY * 0.004, 3.7, 10);
  },
  { passive: false }
);
...
function animate(now) {
  ...
  if (playing) {
    const speed = Number(ui.growthSpeed?.value || 1);
